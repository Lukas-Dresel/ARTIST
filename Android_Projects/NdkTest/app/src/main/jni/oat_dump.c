...
void log_dex_file_class_def_contents(const struct DexHeader* hdr, uint16_t class_def_index)
{
    ...
    const char* super_class_name = !IsValidIndex(c.superclass_idx_) ? "[INVALID CLASS INDEX]" : dex_file_GetTypeIdName(hdr, c.superclass_idx_).content;
...
void log_dex_file_method_id_array_contents_by_class_def_index(const struct DexHeader *hdr, uint16_t class_def_index)
{
    ...
    const char* class_name = GetStringdex_file_GetStringDataByIndex(hdr, dex_file_GetTypeDescriptorByIndex(hdr, c.class_idx_).descriptor_idx_).content;
...
void log_dex_file_class_data_contents(const struct DexHeader* hdr, uint32_t class_def_index)
{
    LOGD("Class Data: ");
