...
bool oat_FindDirectMethod(const struct OatClass * oat_class, struct OatMethod* result, const char* descriptor, const char* signature)
{
     ...
     */
    const struct OatMethodOffsets* meth_off = GetOatMethodOffsets(&oat_class->oat_class_data, result->dex_method.decoded_method_data.method_idx);
...
{
    return (void*)EntryPointToCodePointer(oat_GetQuickCodeCompiledEntryPoint(m));
}
