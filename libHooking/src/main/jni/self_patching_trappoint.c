 ...
 *
 *
 * TODO: MUST BE REWORKED TO AVOID MEMORY LEAKS
 *
 *
 *
 *
...
bool SelfPatchingTrappoint_Disable(void *p)
{
    ...
    {
        trappoint_Disable(self->primary_trappoint);
    }
    if(self->patch_trappoint != NULL)
    {
        trappoint_Disable(self->patch_trappoint);
    }
...
void SelfPatchingTrappoint_Uninstall(void *p)
{
    ...
    if(self->primary_trappoint != NULL)
    {
        trappoint_Destroy(self->primary_trappoint);
    }
    if(self->patch_trappoint != NULL)
    {
        trappoint_Destroy(self->patch_trappoint);
    }
