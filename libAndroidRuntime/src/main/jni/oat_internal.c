...
const char *GetStoreValueByKey(const struct OatHeader *self, const char *key)
{
    const char *ptr = (const char *) (&self->key_value_store_);
    const char *end = ptr + self->key_value_store_size_;
    while (ptr < end)
    {
...
bool GetStoreKeyValuePairByIndex(const struct OatHeader *self, size_t index, const char **key,
                                     const char **value)
{
    const char *ptr = (const char *) (&self->key_value_store_);
    const char *end = ptr + self->key_value_store_size_;
...
bool ReadOatValue_String(const void **data, void* end, struct String *result)
{
    ...
    // Overwrite the data pointer to point after the string.
    *data = p;
    return true;
}
...
bool DecodeOatClassData(void* oat_class_pointer, void* end, struct OatClassData * result)
{
    ...
    int16_t     class_status    = 0;
    uint16_t    class_type      = 0;

    const byte* status_pointer = oat_class_pointer;
    const byte* type_pointer = status_pointer + sizeof(class_status);
    const byte* after_type_pointer = type_pointer + sizeof(class_type);
    ...
    class_status    = *(int16_t*)  status_pointer;
    ...
    class_type      = *(uint16_t*) type_pointer;
    ...
    result->class_status = class_status;
...
const struct OatMethodOffsets* GetOatMethodOffsets(const struct OatClassData* clazz, uint32_t class_method_index)
{
    // Sadly no checks on the class_method_index are possible because
    // the length of the method list is not kept
    CHECK_RETURNNULL(clazz != NULL);
    if(clazz->methods_pointer == NULL)
    {
        CHECK_EQ(kOatClassNoneCompiled, clazz->class_type);
        return NULL;
    }
    ...
    {
        CHECK_EQ(kOatClassSomeCompiled, clazz->class_type);
        const uint32_t* bitmap_ = (uint32_t*)clazz->bitmap_pointer;
        if(!bit_vector_IsBitSet(bitmap_, class_method_index))
        {
            return NULL;
        }
        size_t num_set_bits = bit_vector_NumSetBits(bitmap_, class_method_index);
...
uint32_t GetOatQuickMethodHeaderOffset(void* base, const struct OatMethodOffsets* off)
{
    ...
    return (uint32_t)((const void*)method_header - base);
}
